...
export const saveDocOnOpenSearch = async (
  client: Client,
...
export const deleteDocOnOpenSearch = async (
  client: Client,
  indexMapping: Record<string, string>,
  data: Record<string, unknown>,
  pk: string
...
export const searchByOpenSearchIndexes = async <T extends boolean>(
  client: Client,
  indexMapping: Record<string, string>,
  ...
  return Object.fromEntries(
    await Promise.all(
      Object.entries(mapping).map(async ([key, val]) => {
        ...
        return [
          key,
          res.body.hits.hits.map(({ _id }: { _id: string }) =>
            pkIsNumber ? Number(_id) : _id
          ),
        ];
      })
    )
  ...
  async (params, next) => {
    ...
    if (
      ...
      params.args.where = getNewWhereArg(
        params.args.where,
        await searchByOpenSearchIndexes(
          client,
          indexMapping,
    ...
    if (params.action === "create" && options?.syncOn?.includes("create")) {
      ...
      await saveDocOnOpenSearch(client, indexMapping, record, pk);
      return record;
    }
    ...
    if (params.action === "delete" && options?.syncOn?.includes("delete")) {
      const record = await next(params);
      await deleteDocOnOpenSearch(client, indexMapping, record, pk);
      return record;
    }
